...
public partial class frmResponsavelEtapaPesquisa : SortingPageBase
{
    ...
    {
        List<StatusLicitacao> list = StatusLicitacao.Select();
		this.Sort(list);
    ...
    void gvPesquisa_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if(e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            StatusLicitacao status = (StatusLicitacao)e.Row.DataItem;
            string address = "frmStatusLicitacaoResponsavel.aspx";
                
            
            Anthem.AnthemClientMethods.Redirect(address + "?id_status=" + status.ID, link);
        }
    }
