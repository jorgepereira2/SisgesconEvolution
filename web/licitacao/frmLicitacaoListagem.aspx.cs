...
public partial class frmLicitacaoListagem : SortingPageBase
{
    ...
    {
        if(e.Row.RowType == DataControlRowType.DataRow && gvPesquisa.Columns[4].Visible)
        {
            DataGrid dg = (DataGrid) e.Row.FindControl("dgItem");
            Licitacao licitacao = (Licitacao) e.Row.DataItem;
            dg.DataSource = licitacao.Itens;
            dg.DataBind();
        }
        ...
        if(e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if((Request["Validade"] == "1" && licitacao.DataPregao.HasValue && licitacao.DataPregao.Value.AddYears(1) < DateTime.Today) ||
                (Request["Validade"] == "2" && licitacao.DataPregao.HasValue && licitacao.DataPregao.Value.AddYears(1) >= DateTime.Today))
                e.Row.Visible = false;
