...
public partial class frmResponsavelEtapaPesquisa : SortingPageBase
{
    ...
    {
		List<StatusPedidoAquisicao> list = StatusPedidoAquisicao.Select();
		this.Sort(list);
    ...
    void gvPesquisa_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if(e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            StatusPedidoAquisicao status = (StatusPedidoAquisicao) e.Row.DataItem;
            string address = "";
            if(status.FlagVinculoPorDivisao)
                address = "frmStatusPedidoAquisicaoDivisao.aspx";
            else
                address = "frmStatusPedidoAquisicaoResponsavel.aspx";
