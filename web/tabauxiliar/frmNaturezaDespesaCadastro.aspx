<%@ Page Language="C#" AutoEventWireup="true" CodeFile="frmNaturezaDespesaCadastro.aspx.cs" Inherits="frmNaturezaDespesaCadastro" %>
...
<body>
    ...
    <br />
        Cadastro de Natureza de Despesa
                                 ...
                                 <Anthem:DataGrid runat="server" ID="dgCadastro" Width="98%" CssClass="datagrid"
                                    ...
                                    <Columns>
                                        <asp:TemplateColumn HeaderText="Código" sortexpression="Codigo">
                                            <ItemTemplate>
                                                <%# ((NaturezaDespesa)Container.DataItem).Codigo %>
                                            </ItemTemplate>
                                            <EditItemTemplate>
                                                <Anthem:TextBox  runat="server" ID="txtCodigo" columns="4" MaxLength="12" 
                                                    Text=' <%# ((NaturezaDespesa)Container.DataItem).Codigo %>' />
                                                <Anthem:RequiredFieldValidator runat="server" ControlToValidate="txtCodigo" 
                                                     Display="Dynamic" ValidationGroup="Descricao" ErrorMessage="Campo Obrigatório" /> 
                                            </EditItemTemplate>
                                            <FooterTemplate>
                                                <Anthem:TextBox runat="server" ID="txtCodigoNovo" columns="40" MaxLength="12" />
                                                <Anthem:RequiredFieldValidator runat="server" ControlToValidate="txtCodigoNovo" 
                                                     Display="Dynamic" ValidationGroup="Descricao" ErrorMessage="Campo Obrigatório" /> 
                                            </FooterTemplate>
                                        </asp:TemplateColumn>
                                        <asp:TemplateColumn HeaderText="Descrição" sortexpression="Descricao">
                                            <ItemTemplate>
                                                <%# ((NaturezaDespesa)Container.DataItem).Descricao %>
                                            </ItemTemplate>
                                            <EditItemTemplate>
                                                <Anthem:TextBox  runat="server" ID="txtDescricao" columns="40" Text=' <%# ((NaturezaDespesa)Container.DataItem).Descricao %>' />
                                        ...
                                        <asp:TemplateColumn HeaderText="Tipo Natureza" sortexpression="TipoNaturezaDespesa">
                                            <ItemTemplate>
                                                <%# Shared.Common.Util.GetDescription((((NaturezaDespesa)Container.DataItem).TipoNaturezaDespesa)) %>
                                            </ItemTemplate>
                                            <EditItemTemplate>
                                                <Anthem:DropDownList  runat="server" ID="ddlTipoNaturezaDespesa" />                                                
                                            </EditItemTemplate>
                                            <FooterTemplate>
                                                <Anthem:DropDownList  runat="server" ID="ddlTipoNaturezaDespesaNovo" />
                                            </FooterTemplate>
                                        </asp:TemplateColumn> 

                                         <asp:TemplateColumn HeaderText="Obs." sortexpression="Observacao">
                                            <ItemTemplate>
                                                <%# ((NaturezaDespesa)Container.DataItem).Observacao %>
                                            </ItemTemplate>
                                            <EditItemTemplate>
                                                <Anthem:TextBox  runat="server" ID="txtObservacao" columns="60" Text=' <%# ((NaturezaDespesa)Container.DataItem).Observacao %>' />                                               
                                            </EditItemTemplate>
                                            <FooterTemplate>
                                                <Anthem:TextBox runat="server" ID="txtObservacaoNovo" columns="60" />                                               
                                            </FooterTemplate>
                                        </asp:TemplateColumn>
                                         <asp:TemplateColumn HeaderText="Ativo" ItemStyle-HorizontalAlign="Center" >
                                            <ItemTemplate>
                                                <%# ((NaturezaDespesa)Container.DataItem).FlagAtivo ? "Sim" : "Nao" %>
                                            </ItemTemplate>
                                            <EditItemTemplate>
                                                <Anthem:CheckBox runat="server" ID="chkAtivo" Text="Ativo" checked='<%# ((NaturezaDespesa)Container.DataItem).FlagAtivo %>' />                                                
                                            </EditItemTemplate>
                                            <FooterTemplate>
                                        ...
                                        <asp:TemplateColumn HeaderText="" ItemStyle-HorizontalAlign="Center">
                                            <ItemTemplate>
                                                <a href="#" onclick='javascript:Excluir(<%# ((NaturezaDespesa)Container.DataItem).ID %>)' >
