<%@ Page Language="C#" AutoEventWireup="true" CodeFile="frmAtividadeCadastro.aspx.cs" Inherits="frmAtividadeCadastro" %>
                                 ...
                                 <Anthem:DataGrid runat="server" ID="dgCadastro" Width="98%" CssClass="datagrid"
                                    ...
                                    <Columns>                                        
                                        <asp:TemplateColumn HeaderText="Descrição" sortexpression="Descricao">
                                            <ItemTemplate>
                                                <%# ((Atividade)Container.DataItem).Descricao %>
                                            </ItemTemplate>
                                            <EditItemTemplate>
                                                <Anthem:TextBox  runat="server" ID="txtDescricao" columns="40" Text=' <%# ((Atividade)Container.DataItem).Descricao %>' />
                                         ...
                                         <asp:TemplateColumn HeaderText="Apropriação" sortexpression="Apropriacao">
                                            <ItemTemplate>
                                                <%# ((Atividade)Container.DataItem).Apropriacao.Descricao %>
                                            </ItemTemplate>
                                            <EditItemTemplate>
                                                <Anthem:DropDownList runat="server" ID="ddlApropriacao" />
                                                <Anthem:RequiredFieldValidator runat="server" ControlToValidate="ddlApropriacao" InitialValue="0" 
                                                     Display="Dynamic" ValidationGroup="Descricao" ErrorMessage="Campo Obrigatório" /> 
                                            </EditItemTemplate>
                                            <FooterTemplate>
                                                <Anthem:DropDownList runat="server" ID="ddlApropriacaoNovo" />
                                                <Anthem:RequiredFieldValidator  runat="server" ControlToValidate="ddlApropriacaoNovo" InitialValue="0" 
                                                     Display="Dynamic" ValidationGroup="Descricao" ErrorMessage="Campo Obrigatório" /> 
                                            </FooterTemplate>
                                        </asp:TemplateColumn>
                                        <asp:TemplateColumn HeaderText="Atividade Direta" ItemStyle-HorizontalAlign="Center" >
                                            <ItemTemplate>
                                                <%# ((Atividade)Container.DataItem).FlagAtividadeDireta ? "Sim" : "Nao" %>
                                            </ItemTemplate>
                                            <EditItemTemplate>
                                                <Anthem:CheckBox runat="server" ID="chkAtividadeDireta" Text="Atividade Direta" checked='<%# ((Atividade)Container.DataItem).FlagAtividadeDireta %>' />                                                
                                            </EditItemTemplate>
                                            <FooterTemplate>
                                                <Anthem:CheckBox runat="server" ID="chkAtividadeDiretaNovo" Text="Atividade Direta" Checked="true"/>                                                
                                            </FooterTemplate>
                                        </asp:TemplateColumn>
                                        <asp:TemplateColumn HeaderText="Ativo" ItemStyle-HorizontalAlign="Center" >
                                            <ItemTemplate>
                                                <%# ((Atividade)Container.DataItem).FlagAtivo ? "Sim" : "Nao" %>
                                            </ItemTemplate>
                                            <EditItemTemplate>
                                                <Anthem:CheckBox runat="server" ID="chkAtivo" Text="Ativo" checked='<%# ((Atividade)Container.DataItem).FlagAtivo %>' />                                                
                                            </EditItemTemplate>
                                            <FooterTemplate>
                                        ...
                                        <asp:TemplateColumn HeaderText="" ItemStyle-HorizontalAlign="Center">
                                            <ItemTemplate>
                                                <a href="#" onclick='javascript:Excluir(<%# ((Atividade)Container.DataItem).ID %>)' >
