...
public partial class frmStatusPedidoObtencaoDivisao : MarinhaPageBase
{
    #region Private Mambers

    [TransientPageState]
    protected StatusPedidoObtencao _status;
    ...
    void ucMessageBox_MessageBoxClose(object sender, MessageBoxEventArgs e)
    {
        ...
        {
            StatusPedidoObtencaoDivisao divisao = _status.ResponsaveisDivisao.Find(Convert.ToInt32(e.Data));
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!this.IsPostBack)
        {
            _status = StatusPedidoObtencao.Get(Convert.ToInt32(Request["id_status"]));
    ...
    void dgCadastro_ItemCommand(object source, DataGridCommandEventArgs e)
    {
        if (e.CommandName == "Insert")
        {
            try
            {
                ...
                StatusPedidoObtencaoDivisao responsavelDivisao = new StatusPedidoObtencaoDivisao();
                ...
                responsavelDivisao.StatusPedidoObtencao = _status;
    ...
    void dgCadastro_ItemDataBound(object sender, DataGridItemEventArgs e)
    {
        if(e.Item.ItemType == ListItemType.Footer)
        {
            ...
            //if (Convert.ToInt32(Request["id_status"]) == Convert.ToInt32(StatusPedidoObtencaoEnum.AguardandoAprovacaoEncarregadoDivisao)
            //    || Convert.ToInt32(Request["id_status"]) == Convert.ToInt32(StatusPedidoObtencaoEnum.AguardandoEnvioEmpenho)
            //    || Convert.ToInt32(Request["id_status"]) == Convert.ToInt32(StatusPedidoObtencaoEnum.AguardandoEntregaExecucao))
            //    Util.FillDropDownList(ddlCelula, Celula.List(TipoCelula.Divisao, null));
            if (Convert.ToInt32(Request["id_status"]) == Convert.ToInt32(StatusPedidoObtencaoEnum.AprovacaoEncDivisão))
                Util.FillDropDownList(ddlCelula, Celula.List(TipoCelula.Divisao, null));
            else
                Util.FillDropDownList(ddlCelula, Celula.List(TipoCelula.Departamento, null));
        }
    }
