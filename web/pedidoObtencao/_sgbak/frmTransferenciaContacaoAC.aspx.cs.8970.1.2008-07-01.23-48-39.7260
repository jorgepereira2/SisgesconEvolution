...
public partial class frmTransferenciaContacaoAC : SortingPageBase
{
    ...
    {
        List<ICompra> list = PedidoCotacao.SelectParaTransferencia(
    ...
    private void DgPesquisa_OnUpdateCommand(object source, DataGridCommandEventArgs e)
    {
        ...
        ICompra compra;
        if(lblTipo.Text == "AC")
            compra = AutorizacaoCompra.Get(id);
        else
            compra = PedidoCotacao.Get(id);
        DropDownList ddlNovoComprador = (DropDownList)e.Item.FindControl("ddlNovoComprador");
        compra.Servidor = Servidor.Get(Convert.ToInt32(ddlNovoComprador.SelectedValue));
        compra.Save();
    ...
    private void DgPesquisa_OnItemDataBound(object sender, DataGridItemEventArgs e)
    {
        ...
        {
            DropDownList ddlNovoComprador = (DropDownList)e.Item.FindControl("ddlNovoComprador");
            ICompra compra = (ICompra)e.Item.DataItem;

            Util.FillDropDownList(ddlNovoComprador, Servidor.List(FuncaoServidor.Comprador));
            ddlNovoComprador.SelectedValue = compra.Servidor.ID.ToString();
        }
    }
}
